...
351 top-level definitions in 30 modules. `utils/geotizer_orchestration.py` is gone;
...
de-coupling the evidence core from the GeoTeaser cell: **55 of the 351
definitions still mention `field_key`**, thirteen of them inside
`project_evidence/`.

Only those thirteen are the residue. Of the other 42, **39** are in
