...
359 top-level definitions in 31 modules. `utils/geotizer_orchestration.py` is gone;
...
de-coupling the evidence core from the GeoTeaser cell: **56 of the 359
definitions still mention `field_key`**, thirteen of them inside
`project_evidence/`.

Only those thirteen are the residue. Of the other 43, **40** are in
