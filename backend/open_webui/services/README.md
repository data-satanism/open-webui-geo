...
405 top-level definitions in 31 modules. `utils/geotizer_orchestration.py` is gone;
...
de-coupling the evidence core from the GeoTeaser cell: **70 of the 405
definitions still mention `field_key`**, fourteen of them inside
`project_evidence/`.

Only those fourteen are the residue. Of the other 56, **53** are in
