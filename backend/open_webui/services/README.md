...
386 top-level definitions in 31 modules. `utils/geotizer_orchestration.py` is gone;
...
de-coupling the evidence core from the GeoTeaser cell: **63 of the 386
definitions still mention `field_key`**, fourteen of them inside
`project_evidence/`.

Only those fourteen are the residue. Of the other 49, **46** are in
