"""`normalize_source_inventory` repairs, deduplicates and remaps an owner's source inventory."""
...
def _export_is_a_shim(export_path) -> bool:
    """Return True when the Workspace export is missing, or mentions `fill_geotizer` and
    has fewer than 500 lines."""
...
def test_an_evidence_shaped_source_is_rebuilt_rather_than_dropped():
    """An entry in the evidence schema is rebuilt with `source_type`, `title` and
    `locator` rather than dropped."""
...
def test_the_domain_decides_the_source_type(domain, expected):
    """`source_domain` decides `source_type`, with `derived` as the fallback."""
...
def test_an_entry_with_no_source_id_is_dropped_and_counted():
    """An entry without `source_id` is dropped and counted in a note."""
    fixed, notes = normalize_source_inventory(
...
def test_two_entries_with_the_same_content_are_merged_despite_different_ids():
    """Entries with the same content merge into the first despite different ids."""
    fixed, notes = normalize_source_inventory(
        _envelope(
            [
...
def test_a_patch_pointing_at_a_merged_entry_is_remapped():
    """A patch's `source_refs` are remapped onto the surviving entry."""
    fixed, _ = normalize_source_inventory(
        _envelope(
            [
...
def test_two_sources_that_differ_only_by_url_stay_separate():
    """Entries that differ only by `url` stay separate."""
    fixed, notes = normalize_source_inventory(
        _envelope(
            [
...
def test_a_well_formed_inventory_produces_no_notes():
    """A well-formed inventory produces no notes."""
    fixed, notes = normalize_source_inventory(
...
def reference():
    """Return `normalize_source_inventory` executed from the Workspace export with its
    `_DOMAIN_TO_SOURCE_TYPE` constant; skip when the export is a shim."""
...
def test_the_port_repairs_exactly_what_the_deployed_tool_repairs(reference, envelope):
    """The port and the export return the same envelope and notes for the same input."""
