"""Tests that `chunk_marker` reads both chunk shapes and that `stamp_chunk_provenance` puts the chunk and its failed
contributors on every patch without refusing any.
"""
...
class TestOneShapeForAChunk:
    """`chunk_marker` reads a chunk given as a mapping or as an `index/total` string into one shape."""
...
class TestTheStampReachesEveryCellOfTheChunk:
    ...
    def test_a_chunk_that_lost_nothing_carries_no_marker(self):
        """A chunk with no failed contributor gets `owner_chunk` and no `evidence_incomplete`."""
    ...
    def test_the_order_is_stable_so_two_runs_compare(self):
        """`evidence_incomplete` has the same order whatever order the failures arrive in."""
    ...
    def test_the_original_envelope_is_not_mutated(self):
        """`stamp_chunk_provenance` leaves its input envelope unchanged."""
        source = envelope()
...
def test_a_filled_cell_keeps_its_value_and_its_status():
    """A stamped `filled` cell keeps its status and value."""
    stamped = stamp_chunk_provenance(
...
def test_a_not_found_cell_is_marked_too():
    """A `not_found` cell is stamped with `evidence_incomplete` too."""
    stamped = stamp_chunk_provenance(
...
class TestTheLogAnswersPerChunk:
    ...
    def test_the_index_is_uncapped_so_a_long_run_still_marks_its_cells(self):
        """`failures_for` answers for chunks whose records the capped list dropped."""
