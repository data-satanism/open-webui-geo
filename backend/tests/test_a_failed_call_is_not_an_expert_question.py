"""A cell with no usable owner answer gets `agent_contract_failed` where the service accepts it, not expert review."""
...
def test_a_service_that_has_not_heard_of_it_gets_the_old_status():
    """A batch that does not advertise `agent_contract_failed` gets `requires_expert_review`."""
...
def test_assemble_keeps_the_expert_status_even_where_the_new_one_is_offered():
    """The `ASSEMBLE` batch gets `requires_expert_review` even when it advertises the new status."""
    assert (
...
def test_the_fallback_still_says_why():
    """The fallback envelope's retrieval note still states that no usable envelope arrived."""
...
def test_this_repository_accepts_the_envelope_it_writes():
    """`validate_owner_envelope` accepts the fallback envelope carrying `agent_contract_failed`."""
...
def test_a_run_whose_owner_never_answers_ends_in_the_new_status():
    """`_produce_valid_owner_envelope` with an owner that never answers ends
    with every patch `agent_contract_failed`."""
...
def test_a_contract_failure_may_not_carry_a_value():
    """A patch with status `agent_contract_failed` and a non-null value is refused."""
    value = _batch(advertises=True)
