...
| `test_each_download_is_authenticated` (`test_geotizer_boundary_contract.py`) | Fixed-width text windows (600 characters) of `routers/geotizer.py` | Code within those windows. |
...
| `test_every_artefact_the_area_links_is_one_the_wrapper_can_serve`, `test_the_wrapper_serves_nothing_it_has_no_mapping_for` (`test_the_area_links_to_its_own_files.py`) | An AST scan of `routers/geotizer.py` (`PROXY_SOURCE`), through `_proxy_artifacts` | The `ARTIFACTS` mapping and the route paths. |
