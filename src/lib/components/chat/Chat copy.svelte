<script lang="ts">
	...
	type ChatQueueItem = { id: string; prompt: string; files: any[] };

	const getChatQueue = (targetChatId: string) => $chatRequestQueues[targetChatId] ?? [];

	const setChatQueue = (targetChatId: string, queue: ChatQueueItem[]) => {
		chatRequestQueues.update((queues) => {
			if (queue.length === 0) {
				const { [targetChatId]: _removed, ...rest } = queues;
				return rest;
			}
	...
	const navigateHandler = async () => {
		...
		if ($chatId) {
			const currentQueue = getChatQueue($chatId);
			if (currentQueue.length > 0) {
				sessionStorage.setItem(`chat-queue-${$chatId}`, JSON.stringify(currentQueue));
			}
		}
		...
		if (chatIdProp && (await loadChat())) {
			...
			if (storedQueueData) {
				try {
					const restoredQueue: ChatQueueItem[] = JSON.parse(storedQueueData);
					if (restoredQueue.length > 0) {
						setChatQueue(chatIdProp, restoredQueue);
						...
						{#if ($settings?.landingPageMode === 'chat' && !$selectedFolder) || createMessagesList(history, history.currentId).length > 0}
							...
							<div class=" pb-2 {dragged ? 'z-0' : 'z-10'}">
								<MessageInput
									...
									{createMessagePair}
									{onUpload}
									messageQueue={getChatQueue($chatId)}
									onQueueSendNow={async (id) => {
										const queue = getChatQueue($chatId);
										const item = queue.find((m) => m.id === id);
										if (item) {
											// Remove from queue
											setChatQueue($chatId, queue.filter((m) => m.id !== id));
